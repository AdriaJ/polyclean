...
class GaussPolyCLEAN(polyclean.PolyCLEAN):
    def __init__(self,
                 ...
                 norm_kernels: int = 2,
                 **kwargs):
        super().__init__(uvwlambda, direction_cosines, data, **kwargs)
...
def plot_3_images(
    ...
    axes = fig.subplots(1, 3, sharex=True, sharey=True, subplot_kw={'projection': im_list[0].image_acc.wcs.sub([1, 2]),
                                                                    'frameon': False})
    for i in range(3):
        ax = axes[i]
        ...
        if neg := (arr < 0).any():
            ...
            aximn = ax.imshow(-im_neg, origin="lower", cmap="CMRmap_r", interpolation='none', alpha=0.8,
                              vmin=vmin, vmax=vmax)
            any_neg = True
        aximp = ax.imshow(im_pos, origin="lower", cmap=cm, interpolation='none', vmax=vmax, vmin=vmin)
        if i == 0:

...
if __name__ == "__main__":
    ...
    print(f"PolyCLEAN: Solving for lambda factor {lambda_factor:.2f} ...")
    ...
    dual_certif_im.pixels.data = pclean_residual.reshape(dual_certif_im.pixels.data.shape) / lambda_

    ## Save the reconstructions
    folder_path = os.path.join(os.getcwd(), 'certif_pkl', str(lambda_factor))
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    with open(os.path.join(folder_path, "restored.pkl"), 'wb') as handle:
        pickle.dump(pclean_restored, handle)
    with open(os.path.join(folder_path, "certificate.pkl"), 'wb') as handle:
